...
class CBRatingSystemAdmin
{
    ...
    public static function init()
    {
        ...
        //set screen options for user log and rating avg page

        add_filter( 'set-screen-option', array( 'CBRatingSystemAdmin', 'cbrating_set_option_user_log' ), 10, 3 );
        add_filter( 'set-screen-option', array( 'CBRatingSystemAdmin', 'cbrating_set_option_avg_log' ), 10, 3 );
    ...
    public static function admin_rating_mainmenu()
    {
        ...
        $ratingactiontop     = add_menu_page(CB_RATINGSYSTEM_PLUGIN_NAME . ' Main', CB_RATINGSYSTEM_PLUGIN_NAME, 'administrator', 'rating', array('CBRatingSystemAdminDashboard', 'build_admin_dashboard'), plugins_url('images/admin_icon/rating-home-16.png', __FILE__));
        ...
        add_action( "load-$ratingactionuserlog", array( 'CBRatingSystemAdmin', 'cbrating_add_option_userlog' ) );
        ...
        //add screen option for affiliators wp list table
        add_action( "load-$ratingactionavglog", array( 'CBRatingSystemAdmin', 'cbrating_add_option_avg' ) );
    ...
    /**
     * Add screen option for user log listing
     */
    public static function cbrating_add_option_userlog() {
        $option = 'per_page';
        $args   = array(
            'label'   => __( 'Number of items per page:', 'cbratingsystem' ),
            'default' => 50,
            'option'  => 'user_log_per_page'
        );
        add_screen_option( $option, $args );
    }

    /**
     * Add screen option for avg rating listing
     */
    public static function cbrating_add_option_avg() {
        $option = 'per_page';
        $args   = array(
            'label'   => __( 'Number of items per page:', 'cbratingsystem' ),
            'default' => 50,
            'option'  => 'avg_log_per_page'
        );
        add_screen_option( $option, $args );
    }

    /**
     * Set options for user rating log.
     *
     * @param $status
     * @param $option
     * @param $value
     ...
     * @return mixed
     */
    public static function cbrating_set_option_user_log( $status, $option, $value ) {
        if ( 'user_log_per_page' == $option ) {
            return $value;
        }
     ...
     * @return mixed
     */
    public static function cbrating_set_option_avg_log( $status, $option, $value ) {
        if ( 'avg_log_per_page' == $option ) {
            return $value;
        }
    ...
    public static function admin_load_scripts_and_styles($hook)
    {
        ...
        wp_register_script('cbratingsystem_admin_clipboard', CB_RATINGSYSTEM_PLUGIN_DIR_URL . 'js/clipboard.min.js', array('jquery'), CB_RATINGSYSTEM_PLUGIN_VERSION);
        ...
        wp_enqueue_script('cbratingsystem_admin_clipboard');
    ...
    private static function admin_save_ratingForm_new($ratingFormData, $form_default, $form_question, $form_criteria, $postTypes, $userRoles, $editorUserRoles)
    {
        ...
        if (check_admin_referer('cb_ratingForm_edit_form_nonce_action', 'cb_ratingForm_edit_form_nonce_field')) {
            ...
            if (isset($ratingFormData['custom_criteria']) && !empty($ratingFormData['custom_criteria'])) {
                ...
                if (($selectedStars < 1)) {
                    $errorHappened  = true;
                    $errorText      = __('You must choose at least one STAR from Custom Criteria', 'cbratingsystem') . '</i> label';
                    $errorMessage[] = $errorText;
                    $wp_error->add('input_error', $errorText);
                    $affectedFields[] = 'custom_criteria';
                }
            ...
            <?php
                echo $js = '
                    var ratingFormEnableDisableStateText = '.json_encode(
                        array(
                            '1_hover'   => esc_html__('Disable This', 'cbratingsystem'),
                            '1_normal'  => esc_html__('Enabled', 'cbratingsystem'),
                            '0_hover'   => esc_html__('Enable This', 'cbratingsystem'),
                            '0_normal'  => esc_html__('Disabled', 'cbratingsystem'),
                        )
                    ).';
                ';
            ?>
        ...
        <div class="cbrp-form-listing">
            <form method="post" action="">
                ...
                <div class="stuffbox">
                    <table id="log-results" class="widefat tablesorter">
                        ...
                        <tr>
                            ...
                            <th width="10%" class="manage-column column-shortatg">
                                <?php _e('Shortcode Status', 'cbratingsystem'); ?>
                            </th>
                            <th width="30%"
                        ...
                        <img style="width: 16px;" src="' . plugins_url('images/clippy.png', __FILE__) . '" alt="' . __('Copy to clipboard', 'cbratingsystem') . '"/>
                                    ...
                                    <td>
                                        <?php
                                        echo ($ratingForm->enable_shorttag == 1) ? esc_html__('Shortcode Enabled', 'cbratingsystem'):esc_html__('Shortcode Disabled', 'cbratingsystem');
                                        ?>
                                    </td>
                            ...
                            <?php endforeach; ?>
                        <?php else : echo '<td colspan="7" align="center">'.esc_html__('No Records Found','cbratingsystem').'</td>'; ?>
                        <?php endif; ?>
                        </tbody>
                        <tfoot>
                ...
                <div id="rtp-log-deletion">
                    <h3><?php esc_html_e('Delete Logs Data', 'cbratingsystem'); ?></h3>
                    <table class="widefat">
                        <tr>
                            <td><strong><?php esc_html_e('Delete By:', 'cbratingsystem'); ?></strong></td>
                            <td>
                                <select name="cbrp-form-delete-by">
                                    <option value="selected"><?php esc_html_e('Selected Items','cbratingsystem'); ?></option>
                                </select>
