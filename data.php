...
class CBRatingSystemData
{
    ...
    public static function get_ratings($form_id, $post_id = '', $user_id = '', $is_object = false)
    {
        ...
        if(!empty($post_id)){
	        ...
	        $active_clause  =  is_array($post_id) ? " AND ur.post_id IN(%s)" : " AND ur.post_id=%d";
	    ...
	    if(!empty($user_id)){
		    ...
		    $active_clause .= is_array($user_id) ? " AND ur.user_id IN(%s)" : " AND ur.user_id=%d";
		    $user_id        = is_array($user_id) ? implode(',', $user_id) : $user_id;
		    $prepare_args[] =  $user_id;
	    }


        $sql = $wpdb->prepare("SELECT ur.* FROM $table_name1 ur WHERE ur.form_id=%d $active_clause", $prepare_args);
